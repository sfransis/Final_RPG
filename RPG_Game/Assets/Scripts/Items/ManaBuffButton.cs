...
public class ManaBuffButton : MonoBehaviour, IPointerClickHandler {

	private playerMana ph = playerMana.playerManaInstance;
	...
	public void OnButtonPressManaBuff() {
		slotNum = (int)Char.GetNumericValue(transform.parent.gameObject.name[5]);
		ph.UpdateMaxMana(12.5f);
		playerHotbar.isUsed[slotNum - 1] = false;
		Destroy(transform.gameObject);
	}
}